...
const Optimization = () => {
    ...
    const fetchMetrics = async () => {
        try {
            ...
            const response = await axios.get('/api/v1/ai/optimization/metrics', {
    ...
    const runAutoRestock = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`/api/v1/ai/autonomous/restock/generate?auto_receive=${autoReceive}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
